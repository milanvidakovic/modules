...
//)
{
  ...
  int dr;
	...
	unsigned int rcnt;
	uint8_t cs, *rbuff = buf;
	//FATFS *fs = descriptor; //FatFs;
...
//	remain = fs->fsize - fs->fptr;
	...
	if (length > remain) length = (uint16_t)remain;			/* Truncate btr by remaining bytes */

	while (length)	{									/* Repeat until all data transferred */
